...
describe('cumulative tiers allocation form', () => {
  describe('emptyCumulativeTiersForm', () => {
  ...
  describe('removeCumulativeIntermediateTier', () => {
    it('removes an intermediate tier and ignores an invalid index', () => {
      ...
      expect(removeCumulativeIntermediateTier(form, 0)).toEqual(emptyCumulativeTiersForm());
    });
  });

  describe('setCumulativeIntermediateThreshold', () => {
    it('sets a threshold immutably and ignores an invalid index', () => {
      const form = addCumulativeIntermediateTier(emptyCumulativeTiersForm());

      expect(setCumulativeIntermediateThreshold(form, -1, '40')).toBe(form);
      expect(setCumulativeIntermediateThreshold(form, 0, '40').intermediateTiers).toEqual([
        { upToAmountInEuros: '40', participants: [] },
      ]);
      expect(form.intermediateTiers).toEqual([{ upToAmountInEuros: '', participants: [] }]);
    });
  });

  describe('toggleCumulativeIntermediateParticipant', () => {
    it('toggles a participant immutably and ignores an invalid index', () => {
      const form = addCumulativeIntermediateTier(emptyCumulativeTiersForm());

      expect(toggleCumulativeIntermediateParticipant(form, 3, 'alice@example.com')).toBe(form);
      expect(
        toggleCumulativeIntermediateParticipant(form, 0, 'alice@example.com').intermediateTiers,
      ).toEqual([{ upToAmountInEuros: '', participants: ['alice@example.com'] }]);
      expect(form.intermediateTiers).toEqual([{ upToAmountInEuros: '', participants: [] }]);
    });
  });
  ...
  describe('validateCumulativeTiersForm', () => {
    ...
    it.each([
      ...
      expect(
        validateCumulativeTiersForm(formWithThresholds(thresholds), totalAmountInCents),
      ).toEqual({
        kind: 'tier-order',
  ...
  describe('toCumulativeTiersAllocation', () => {
